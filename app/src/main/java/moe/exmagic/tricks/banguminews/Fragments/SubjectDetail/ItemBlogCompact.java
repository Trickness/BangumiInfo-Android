...
public class ItemBlogCompact extends Fragment {
    ...
    private ImageView mBlogSubmitterHeaderView;
    ...
    public View onCreateView(final LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mBlogSubmitterHeaderView = (ImageView) v.findViewById(R.id.blog_compact_header_view);
        if(mBlog != null) {
            mBlogTitle.setText(mBlog.Title);
            mBlogSummary.setText(mBlog.BlogPreview);
            mBlogSubmitInfo.setText("by " + mBlog.Submitter.UserNickname + " " + mBlog.SubmitDatetime + mBlog.BlogReplyNumber);
            Picasso.with(getActivity()).load(mBlog.Submitter.UserHeaderUrl)
                    .transform(mHeaderTransform)
                    .into(mBlogSubmitterHeaderView);
        }
